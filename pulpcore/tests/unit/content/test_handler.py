...
_IMS_BEFORE = http_date(datetime(2019, 1, 1, tzinfo=dt_timezone.utc).timestamp())
...
def _request(*, ims=None, http_range=None):
    return Mock(
        method="GET",
...
@pytest.mark.parametrize(
    "ims, last_modified, cache_enabled, expect_304",
    [
        (None, _LAST_MODIFIED, False, False),
        (_IMS_AFTER, _LAST_MODIFIED, False, True),
        (_IMS_BEFORE, _LAST_MODIFIED, False, False),
        (_IMS_AFTER, None, False, False),
        (_IMS_AFTER, _LAST_MODIFIED, True, False),
    ],
    ids=["no-ims", "fresh", "stale", "no-timestamp", "cache-on"],
)
async def test_serve_content_artifact_if_modified_since(
    monkeypatch, ims, last_modified, cache_enabled, expect_304
):
    """Filesystem responses stamp Last-Modified; matching IMS is 304 unless the cache is on."""
        ...
        if expect_304:
            with pytest.raises(HTTPNotModified) as exc:
                await handler._serve_content_artifact(
                    ca, {}, _request(ims=ims), last_modified=last_modified
                )
...
async def test_serve_content_artifact_redirect_is_not_304(monkeypatch):
    """Object-storage 302s never get a Pulp Last-Modified, so a matching IMS must not 304."""
    redirect = HTTPFound(
...
async def test_serve_content_artifact_304_beats_unsatisfiable_range(monkeypatch):
    ...
    request = _request(ims=_IMS_AFTER, http_range=_UnsatisfiableRange())
